...
## Stop here - only do problems 1-6 for the homework due 1/25
...
```{r}
mean(carnivores$mean.hra.m2, na.rm = T) #13039918
```

```{r}
mean(herbivores$mean.hra.m2, na.rm = T) #34137012 is larger
```
...
```{r}
owls <- select(filter(homerange, order=="strigiformes"), "order", "family", "genus", "species", "common.name", "mean.mass.g", "log10.mass")
owls
```
...
```{r}
bird_hra <- select(filter(homerange, taxon == "birds"), "common.name", "mean.hra.m2")
bird_hra
max(bird_hra$mean.hra.m2)
```
