...
```{r}
p + geom_point(size=.75)
```

We can color the points by a categorical variable.
```{r}
p + geom_point(aes(color=thermoregulation)) #color for scatterplot, fill for bar plot
```
...
homerange %>% 
  ...
  geom_bar()+
  coord_flip()+
  labs(title="Observations by Taxon", x="Taxonomic Group")
```
...
penguins %>% 
  ...
  geom_bar()+
  coord_flip()+
  labs(title="Observations by Island", x="Island")
```
