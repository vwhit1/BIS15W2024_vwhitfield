...
```{r}
homerange %>% ggplot(aes(class, log10.mass))+geom_boxplot()
```

2. Now, add a layer to this plot that fills color for each box by taxonomic class
```{r}
homerange %>% ggplot(aes(class, log10.mass, fill=class))+geom_boxplot()
```

3. Use `group` to show the range of `log10.mass` by taxon.
```{r}
homerange %>% ggplot(aes(class, log10.mass, group=taxon))+geom_boxplot()
```

4. Use `fill` to show the range of `log10.mass` by taxon with color codes.
```{r}
homerange %>% ggplot(aes(class, log10.mass, fill=taxon))+geom_boxplot()
```
...
deserts %>% 
  filter(species_id=="DM" | species_id=="DS") %>% 
  mutate(year=as.factor(year)) %>% #instead of as numeric, which makes x axis continuous and so only every 5 years are plotted - with this, every year is
  group_by(year, species_id) %>% 
  summarize(n=n(), .groups='keep') %>% 
  ggplot(aes(x=year, y=n, group=species_id, color=species_id))+
  geom_line()+
  geom_point(shape=5)+ #can experiment with shapes - 2 is optional
...
homerange %>% 
  ...
  geom_bar(position="dodge", alpha=0.4, color="white")
```
