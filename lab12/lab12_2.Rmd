...
```
```{r}
display.brewer.pal(5,"PuBuGn") #WHOOOAAAAAAAA
```
...
```{r}
q+scale_colour_brewer(palette="Spectral")
```
...
```{r}
my_palette <- paletteer_d("palettetown::oddish") #store your palette
#POKEMON???????????????
```
