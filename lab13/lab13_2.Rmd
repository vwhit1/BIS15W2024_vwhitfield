...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, #pipes will break it! use specific convention
...
```{r}
map2 <- get_stadiamap(bbox_sharks, maptype = "stamen_terrain", zoom=6)
```

```{r}
ggmap(map2) + 
  geom_point(data = sharks_dups, #pipes will break it! use specific convention
             aes(longitude, latitude), size=0.4) +
...
```{r}
sharks_fatal_plot <- filter(sharks_dups, injury=="fatal")
ggmap(map2) + 
  geom_point(data = sharks_fatal_plot, #pipes will break it! use specific convention
             aes(longitude, latitude), size=0.4) +
  labs(x= "Longitude", y= "Latitude", title="Shark Fatality Locations")
```
