...
```
```{r}
homerange %>% group_by(taxon) %>% count()
```
...
ui <- fluidPage(
    ...
    selectInput("filter_taxon", "Filter by", choices = c("birds", "lake fishes", "lizards", "mammals", "marine fishes", "river fishes", "snakes", "tortoises", "turtles"), 
              selected = "mammals"),
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
      ...
      homerange %>% 
        filter(taxon==input$filter_taxon) %>% #the reactive part is the filter
        ggplot(aes(x=log10.hra))+
        geom_density(fill="steelblue", color="black", alpha=0.6)
...
ui <- dashboardPage(
  ...
  dashboardBody(
  fluidRow(
  box(width=3, #<- optional, it'll auto-decide otherwise
  ...
  dashboardBody(
    fluidRow(
      box(title = "Plot Options", width = 3,
        ...
        selectInput("filter", "Select Population", choices = unique(wolves$pop)), hr()
      
          ), # close the first box    

      box(title = "Plot of Wolves Data", width = 9,
        plotOutput("plot", width = "800px", height = "600px")
          ) # close the second box
    ) # close the row
  ) # close the dashboard body
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
  wolves %>% 
    filter(sex!="NA", pop==input$filter) %>% 
      ggplot(aes(x=sex, fill=sex))+
      geom_bar()
    })
}
