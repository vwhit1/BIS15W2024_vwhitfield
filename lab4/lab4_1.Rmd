...
```{r}
select_if(fish, ~!is.numeric(.))
#The ~ is "look in all cells," and the ! is not
```
...
```{r}
summary(mammals) #why is there a -999 value? that's not right....
```
...
mammals_new <- rename(mammals, wean_mass="wean mass", max_life="max. life", litter_size="litter size", litters_per_year="litters/year")
names(mammals_new)
```
...
```{r}
#I did this earlier in the code! But the following is a more efficient way of doing it
...
```
```{r}
mammals <- clean_names(mammals) #thank you janitor for cleaning this up
names(mammals)
```
