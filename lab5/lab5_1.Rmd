...
```{r}
names(mammals) #bringing up the new names for reference
```
...
```{r}
compare_mammals <- filter(select(mammals, "order","genus", "mass", "gestation", "newborn"), genus %in% c("Lophocebus", "Erythrocebus", "Macaca"))
compare_mammals
```

```{r}
min(compare_mammals$newborn)
```
