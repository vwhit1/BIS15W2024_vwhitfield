...
fish %>% # work with the fish data
  select(lakeid, radii_length_mm) %>% #pull out variables (columns) 
  filter(lakeid %in% c("AL", "AR")) %>% #only these lakes
  filter(between(radii_length_mm, 2, 4)) %>% #between 2 and 4 mm
  arrange(desc(radii_length_mm)) #sort to make easier to interpret
```
